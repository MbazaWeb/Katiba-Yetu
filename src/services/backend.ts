...
<head>
<meta charset="utf-8">
<title>${constitution.name[lang] ?? constitution.name.sw} — ${version.name}</title>
<style>
...
<body>
<h1>${constitution.name[lang] ?? constitution.name.sw}</h1>
<div class="meta">
  <strong>${lang === 'sw' ? 'Toleo' : 'Version'}:</strong> ${version.name} (${version.versionNumber})<br>
  <strong>${lang === 'sw' ? 'Tarehe' : 'Date'}:</strong> ${version.publishedAt ? new Date(version.publishedAt).toLocaleDateString() : (lang === 'sw' ? 'Haijachapishwa' : 'Unpublished')}<br>
  <strong>${lang === 'sw' ? 'Sura' : 'Chapters'}:</strong> ${version.totalChapters} ·
  <strong>${lang === 'sw' ? 'Ibara' : 'Articles'}:</strong> ${version.totalArticles}
</div>
<div class="disclaimer">${PROPOSAL_DISCLAIMER}</div>
${chapters.map(chapter => {
  const chapterArticles = articles.filter(a => a.chapterId === chapter.id);
  return `<h2>${lang === 'sw' ? 'Sura' : 'Chapter'} ${chapter.number}: ${chapter.title[lang] ?? chapter.title.sw ?? ''}</h2>
${chapterArticles.map(article => `<div class="article">
  <h3><span class="article-number">${lang === 'sw' ? 'Ibara' : 'Article'} ${article.proposedArticleNumber}:</span> ${article.proposedTitle[lang] ?? article.proposedTitle.sw ?? ''}</h3>
  <p>${article.proposedText[lang] ?? article.proposedText.sw ?? ''}</p>
  ${article.plainLanguageSummary[lang] ? `<p class="plain-language">${article.plainLanguageSummary[lang]}</p>` : ''}
  <p class="source">${lang === 'sw' ? 'Imetengenezwa na' : 'Generated by'}: ${article.generatedBy} · ${lang === 'sw' ? 'Washiriki waliouthibitishwa' : 'Verified participants'}: ${article.verifiedParticipantCount}</p>
</div>`).join('')}`;
}).join('')}
${options.includeMethodology ? `<h2>${lang === 'sw' ? 'Muhtasari wa Mbinu' : 'Methodology Summary'}</h2>
<p>${lang === 'sw' ? 'Rasimu hii imetokana na michango ya wananchi, majadiliano, na kura. Imepitia udhibiti, uainishaji wa mada, na kuunganishwa. Imekaguliwa kisheria na kuidhinishwa na Kamati ya Rasimu.' : 'This draft was generated from citizen submissions, discussions, and polls. It passed moderation, topic classification, and clustering. It was legally reviewed and approved by the Drafting Committee.'}</p>` : ''}
</body>
</html>`;
