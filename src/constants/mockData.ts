import legacyDemoSections from '../data/legacy-demo-sections.json';
import type {
...
/** Legacy homepage fixtures only. Never an official-text source. */
export const MOCK_SECTIONS: Section[] = (legacyDemoSections as Section[])
  .filter(section => section.level === 'chapter')
  .map(chapter => ({ ...chapter, children: (legacyDemoSections as Section[]).filter(article => article.parent_id === chapter.id) }));
